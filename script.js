...
function initTask5() {
  ...
  const results = ['24', '9', '15', '8', '5']; // Ergebnisse von Matheaufgaben
...
function startTask6Game() {
  ...
  function startTask6Timer() {
    let timeLeft = 15; // Setzt den Timer auf 15 Sekunden
  ...
  setTimeout(() => {
    ...
    window.toh.onWin = function() { // Callback function for winning
      ...
      if (taskTitle.includes('Aufgabe 3:')) {
        showTipButtonAfterDelay(45000); // Show the tip button only in task 3 after 45 seconds
      }
    };
  }, 100);
}
