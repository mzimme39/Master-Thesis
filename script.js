...
function initTask5() {
  ...
  const results = ['15', '24', '5', '9', '8']; // Ergebnisse von Matheaufgaben
...
function startTask5Timer() {
  ...
  updateTimerDisplay(timeLeft, timerElement);  // Aktualisiere sofort den Timer
...
const colors = ['red', 'blue', 'green', 'yellow', 'purple']; // Verschiedene Farben für jedes Ziel
const mathResults = {
  '12': 'red',
  '5': 'blue',
  '27': 'green',
...
function startTask6Game() {
  ...
  function startTask6Timer() {
    let timeLeft = 20; // Setzt den Timer auf 20 Sekunden
    updateTimerDisplay(timeLeft, timerElement);
...
function startTask7() {
  ...
  document.getElementById('task7-timer').classList.remove('hidden'); // Zeige den Timer für Aufgabe 4
  startTowersOfHanoi(4); // Starte das Spiel mit 4 Scheiben
...
function startTowersOfHanoi(discs) {
  ...
  setTimeout(() => {
      ...
      window.toh.onWin = function() { // Callback-Funktion für den Gewinnfall
          if (discs === 3) {
              ...
              document.getElementById('to-task4').addEventListener('click', function() {
                  ...
                  showInstructionTask4(); // Zeige Anleitung für Aufgabe 4
              });
          } else if (discs === 4) {
              document.getElementById('to-task8').classList.remove('hidden');
              document.getElementById('to-task8').addEventListener('click', function() {
                  this.classList.add('hidden');
                  showInstructionTask8(); // Zeige Anleitung für Aufgabe 8
              });
          }
      };
  }, 100);
...
class TowersOfHanoi {
  ...
  onWin() {
    ...
    document.getElementById('to-task8').classList.remove('hidden'); // Zeige den Button zur nächsten Aufgabe
  }
  ...
  postWinCleanUp(withMessage) {
    ...
    this.destroyGame(); // Zerstöre das Spiel, sobald es abgeschlossen ist.
    this.onWin(); // Ruft die onWin Callback-Funktion auf
  
    // Überprüfen Sie, ob das Spiel mit 3 Scheiben gewonnen wurde
    if (this.discs === 3) {
      const toTask4Button = document.getElementById('to-task4');
      toTask4Button.classList.remove('hidden');
      toTask4Button.addEventListener('click', () => {
          toTask4Button.classList.add('hidden');
          document.getElementById('task3').classList.add('hidden');
          showInstructionTask4();
      });
    }
  
    // Überprüfen Sie, ob das Spiel mit 4 Scheiben gewonnen wurde
    if (this.discs === 4) {
      const toTask8Button = document.getElementById('to-task8');
      toTask8Button.classList.remove('hidden');
      toTask8Button.addEventListener('click', function() {
          this.classList.add('hidden'); // Verstecken Sie den Button nach dem Klicken
          document.getElementById('task3').classList.add('hidden'); // Zusätzliche Zeile, um das Spiel zu verstecken
          showInstructionTask8();
      });
    }
    ...
    if (this.timer) {
      ...
      document.getElementById('task7-timer').classList.add('hidden'); // Verstecke den Timer
    }
...
function startDrawingTask(level) {
  ...
  canvas.onmouseup = function(e) {
      ...
      if (level === 1) {
        drawing = false;
        // Berechnen Sie den Abstand zum Zielkreis, der sich am Ende des Pfades befindet
        const endCircleX = canvas.width - 20; // X-Koordinate des Zielpunktes
        const endCircleY = canvas.height / 2; // Y-Koordinate des Zielpunktes
        const endCircleRadius = 5; // Radius des Zielpunktes
    
        // Prüfen, ob der Benutzer den Zielpunkt erreicht hat
        if (x >= endCircleX - endCircleRadius && x <= endCircleX + endCircleRadius &&
          ...
          if (drawing && drawingStarted) {
            ...
            const threshold = ctx.lineWidth * 2; // Distanzschwelle für das Schließen des Kreises
            ...
            // Wenn die Maus losgelassen wird ohne dass eine durchgehende Zeichnung stattfand
            alert('Zeichnung wurde nicht abgeschlossen.');
            startDrawingTask(level);
          }
          ...
          if (distanceToEnd <= threshold) {
            messageDiv.classList.remove('hidden'); // Zeige die Nachricht an
...
function drawPath(level) {
    ...
    if (level === 1) {
        ...
        ctx.stroke();
          ...
          ctx.moveTo(squareStartX + houseWidth, squareStartY + houseHeight); // Untere rechte Ecke
          ctx.lineTo(squareStartX, squareStartY + houseHeight); // Untere linke Ecke
      
          // Zeichne den linken vertikalen Balken nach oben
          ctx.lineTo(squareStartX, squareStartY); // Obere linke Ecke
      
          // Zeichne das Dach von der oberen linken Ecke zur Spitze
          ctx.lineTo(squareStartX + houseWidth / 2, squareStartY - roofHeight); // Dachspitze
      
          // Zeichne das Dach von der Spitze zur oberen rechten Ecke
          ctx.lineTo(squareStartX + houseWidth, squareStartY); // Obere rechte Ecke
      
          // Zeichne den rechten vertikalen Balken nach unten
          ctx.lineTo(squareStartX + houseWidth, squareStartY + houseHeight + (pathHeight / 2)); // Erweiterte untere rechte Ecke
      
          ctx.strokeStyle = 'rgba(128, 128, 128, 0.5)';
          ctx.lineWidth = pathHeight;
          ctx.stroke();
          ctx.closePath(); // Schließt den Pfad um eine kontinuierliche Linie zu gewährleisten
      }
...
function startDrawingTask8(level) {
  ...
  canvas8.onmousemove = function(e) {
    if (!drawing) return;
    const rect = canvas8.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    ...
    if (level === 1) {
      ...
      const distance = Math.sqrt(Math.pow(x - canvas8.width / 2, 2) + Math.pow(y - canvas8.height / 2, 2));
      // Angepasst, um den Zielbereich zu vergrößern, indem der äußere Radius weiter nach außen gesetzt wird
      const outerRadius = Math.min(canvas8.width, canvas8.height) / 2 - 10; // Neuer, kleinerer Radius
      const innerRadius = outerRadius - pathHeight; // Innerer Radius wird automatisch angepasst
  ...
  canvas8.onmouseup = function(e) {
      ...
      if (level === 1) {
        drawing = false;
        // Berechnen Sie den Abstand zum Zielkreis, der sich am Ende des Pfades befindet
        const endCircleX = canvas8.width - 20; // X-Koordinate des Zielpunktes
        const endCircleY = canvas8.height / 2; // Y-Koordinate des Zielpunktes
        const endCircleRadius = 5; // Radius des Zielpunktes
        ...
        if (x >= endCircleX - endCircleRadius && x <= endCircleX + endCircleRadius &&
          ...
          if (drawing && drawingStarted) {
            ...
            const y = e.clientY - rect.top;
            const distanceToEnd = Math.sqrt(Math.pow(startCoords.x - x, 2) + Math.pow(startCoords.y - y, 2));
            const threshold = ctx8.lineWidth * 2; // Distanzschwelle für das Schließen des Kreises
        ...
        if (distanceToEnd <= threshold) {
          messageDiv8.classList.remove('hidden'); // Zeige die Nachricht an
